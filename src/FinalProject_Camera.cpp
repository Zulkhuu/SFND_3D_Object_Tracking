...
struct perfStat
{
    ...
    perfStat(string detector, string descriptor, string distance) : 
        ...
        n_matches(0)
        {
            ttc = vector<double>(19,0);
        }
    void print(){
        cout << detectorType << ", " << descriptorType << ", " << distanceType << ", "
               << n_keypoints << ", " << n_matches << ", ";
        for(const auto ttc_ : ttc) {
            cout << ttc_ << ", ";
        }
        cout << endl;
    }
...
int main(int argc, const char *argv[])
{
    ...
    bool save_result = false;     // save perf result to .csv 
    ...
    */
    ...
    if(save_result) {
        ...
        // Column names
        result_file << "Detector-Descriptor-Distance, Number of matches";
        for(int i=1; i<=18; i++)
            result_file << ", TTC #" << i;
        result_file << endl;
        // Result
        for(auto performance : performances) {
            if(performance.n_matches > 0)
